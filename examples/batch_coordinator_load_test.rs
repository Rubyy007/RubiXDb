//! Phase 2B benchmark harness for `execution::batch_coordinator::
//! BatchCoordinatorPool` (Approach B). Same methodology as `examples/
//! leader_drain_load_test.rs`/`worker_pool_load_test.rs` — identical
//! `WalConfig`, identical per-thread record count.
//!
//! Usage: `cargo run --release --example batch_coordinator_load_test --
//! <writer_count> [per_thread=1000]` (no worker_count argument — this
//! architecture always has exactly one coordinator, by design).
...
use rubixdb::execution::batch_coordinator::{
    BatchCoordinatorConfig, BatchCoordinatorPool, Completion,
};
...
fn temp_dir(tag: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("rubixdb_batch_coord_load_test_{tag}_{nanos}"));
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("usage: batch_coordinator_load_test <writer_count> [per_thread=1000]");
        std::process::exit(2);
    }
    ...
    let pool = Arc::try_unwrap(pool).unwrap_or_else(|_| {
        panic!("no other Arc<BatchCoordinatorPool> reference should remain here")
    });
