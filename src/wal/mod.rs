...
// Same reasoning, Phase 5: `crate::manifest`'s own recovery pass needs the
// identical shared/exclusive locking discipline against the *same* `LOCK`
// file the WAL already uses for the whole data directory (RUBIC_MANIFEST_
// FORMAT_SPECIFICATION.md §10 — the Manifest has no lock file of its own).
// Visibility-only; no behavior change to any existing WAL call site.
pub(crate) use file_io::{acquire_exclusive_lock, acquire_shared_lock_if_present, LOCK_FILE_NAME};
